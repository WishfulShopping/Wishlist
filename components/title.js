...
export default function Title({history}) {
  ...
  return (<>
    {isReadWriteUrl() && <Container>
            <Typography sx={{display: {xxs: "none", xs: "none", sm: "none", md: "inline"}}}>Drag and drop the following link to your bookmarks</Typography>
            {bookmarklet && (<Link sx={{color:"tertiary.main", display:"inline"}} href={bookmarklet}>🎁 Add {(name && !edit) && (`to ${name}`)}</Link>)}
